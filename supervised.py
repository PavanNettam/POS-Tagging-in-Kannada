...
#--------------------------------------------------------------------------
tags = [
  'NN', 'NST', 'NNP', 'PRP', 'DEM', 'VM', 'VAUX', 'JJ', 'RB', 'PSP', 'RP',
  'CC', 'WQ', 'QF', 'QC', 'QO', 'CL', 'INTF', 'INJ', 'NEG', 'UT', 'SYM',
  'COMP', 'RDP', 'ECH', 'UNK', 'XC'
]
...
def main():
  ...
  filepath = [
    "./data/hindi_training.txt", "./data/telugu_training.txt",
    "./data/kannada_training.txt", "./data/tamil_training.txt"
  ]
  ...
  file_output = codecs.open(
    "./output/" + languages[int(sys.argv[1])] + "_tags.txt", 'w', 'utf-8')
  file_output.close()
  ...
  for j in range(len(test_input)):
    ...
    for x in range(len(test_words)):
      for y in range(len(tags)):
        ...
        if x > 0:
          max, viterbi_path[y][x] = max_connect(x, y, viterbi_matrix, emission,
                                                transmission_matrix)
        else:
          max = 1
    ...
    file_output = codecs.open(
      "./output/" + languages[int(sys.argv[1])] + "_tags.txt", 'a', 'utf-8')
...
if __name__ == "__main__":
  try:
    ...
    print("Couldn't find the module - {0}, kindly install before proceeding.".format(
      error.message[16:]))
